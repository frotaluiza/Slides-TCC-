...
\begin{slide}{O Problema no Brasil}
  \begin{columns}[T]
    \begin{column}{.55\linewidth}
      \begin{Item}
        \item 2026: ano desafiador — chuvas abaixo da média; Cantareira $\approx$ 30\% (\textcolor{poliRed}{ANA} — Agência Nacional de Águas; \textcolor{poliRed}{CNN Brasil} — canal de notícias)
        \item Nordeste: 4 meses de chuva e 8 meses de seca extrema (ANA)
        \item Dessalinização reduz custo da água em até 10$\times$: R\$ 2,63 a R\$ 4,21/m³ (Confederação Nacional da Indústria — CNI)
...
\begin{slide}{Ilha de Policogeração Sustentável (LabMEMS/COPPE)}
  \begin{Item}
    ...
    \item Dessalinizador V-AGMD: produz 1.000 L/dia de água destilada ($>$ 100 pessoas/dia)
        ...
        \item Em articulação com o \textcolor{poliRed}{\textbf{Projeto Água Doce}}: programa do governo federal que expande o acesso à água potável no Semiárido nordestino por meio de sistemas de dessalinização, atendendo comunidades não cobertas pelo abastecimento convencional
  \end{Item}
...
\begin{slide}{Motivação e Objetivos}
  ...
  \begin{myTitledBox}[Motivação]
    Prever o desempenho do módulo V-AGMD (fluxo de permeado e temperaturas de saída) a partir de dados experimentais (174 pontos, 3 regimes de salinidade). Um modelo físico 0D já publicado pelo laboratório (LISBOA et al., 2024) serve como referência para o sistema.
  \end{myTitledBox}
  \vspace{0.1cm}
  \begin{myTitledBox}[Objetivo geral]
    Avaliar e selecionar modelos de regressão supervisionada para predição do desempenho do V-AGMD, combinando aprendizado a partir de dados com informação física proveniente do modelo reduzido 0D.
  \end{myTitledBox}
  \note{Motivação: prever V-AGMD com 174 pontos experimentais + modelo 0D como referência. Objetivo: avaliar modelos data-driven e híbridos. [~1 min]}
\end{slide}
%========================================================================
