...
\begin{slide}{Fundamentos de Aprendizado de Máquinas}
  ...
  \begin{tabular}{p{0.17\linewidth} p{0.38\linewidth} p{0.38\linewidth}}
      ...
      \bottomrule
    \end{tabular}
  \note{Este trabalho se enquadra no campo da regressão supervisionada: dado um conjunto de variáveis de entrada (condições operacionais do V-AGMD), queremos prever variáveis de saída contínuas (fluxo e temperaturas). A tarefa exige atenção a três aspectos: como medir o erro, como dividir os dados para avaliação justa, e como preparar as variáveis para que modelos diferentes possam ser comparados. Ordem lógica da tabela: 1) métricas (como medir erro), 2) divisão dos dados (como avaliar), 3) validação cruzada (estabilidade), 4) pré-processamento (comparabilidade), 5) correlação (relações entre variáveis). [~1,5 min]}
\end{slide}
