...
\begin{slide}{Busca de Hiperparâmetros}
  ...
  \begin{tabular}{lcp{5.5cm}}
    ...
    \rowcolor{gray!10}
    \makecell{Redes Neurais\\(MLP/Keras)} & \makecell{camadas, neurônios,\\LR, L2, ativação} & \makecell{54 combinações\\(9 arq. × 2 LR × 3 L2)} \\
    \rowcolor{white}
    \makecell{PhyResidual\\PhyHybrid\\PhyInput} & \makecell{L2 apenas\\(arquitetura congelada)} & 3 valores de L2 \\
    \rowcolor{gray!10}
    PhyLoss & \makecell{$\omega$ (ponderação\\da perda física) + L2} & \makecell{$\omega$: 7 valores [0;1]\\+ L2: 5 valores} \\
    \bottomrule
  \end{tabular}
  \vfill
...
\begin{slide}{Validação Cruzada e Seleção 1-SE}
  \centering
  \includegraphics[width=0.60\linewidth]{tcc_images/chap04/criterio_selecao_esquema.png}
